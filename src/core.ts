...
const castBaseSizeToQuoteSize = (baseSize: number, basePrice: number, decimals: number) => floor(baseSize * basePrice, decimals)
// TODO: USE FTX MARKET DECIMALS
const castQuoteSizeToBaseSize = (quoteSize: number, basePrice: number) => floor(quoteSize / basePrice, 3)

export const openHedgedPosition = async (ftxSymbol: string, outputToken: Token, quoteSize: number) => {
  ...
  const { data: firstOrder } = await placeOrder({ symbol: ftxSymbol, price: ask, size: castQuoteSizeToBaseSize(quoteSize, ask) })
  if (!firstOrder) {
    tickerUnsub()
    fillsUnsub()
    return
...
await openHedgedPosition('SOL', SOL, 50)
